...
#define DO_INIT()	DDRB &= ~(1<<4)				/* Initialize port for MMC DO as input */
#define DO			(PINB &	(1<<4))	/* Test for MMC DO ('H':true, 'L':false) */

#define DI_INIT()	DDRB  |= (1<<3)	/* Initialize port for MMC DI as output */
#define DI_H()		PORTB |= (1<<3)	/* Set MMC DI "high" */
#define DI_L()		PORTB &= ~(1<<3)	/* Set MMC DI "low" */

#define CK_INIT()	DDRB  |= 1<<0	/* Initialize port for MMC SCLK as output */
#define CK_H()		PORTB |= 1<<0	/* Set MMC SCLK "high" */
#define	CK_L()		PORTB &= ~(1<<0)	/* Set MMC SCLK "low" */

#define CS_INIT()	DDRB  |= 1<<2	/* Initialize port for MMC CS as output */
#define	CS_H()		PORTB |= 1<<2	/* Set MMC CS "high" */
#define CS_L()		PORTB &= ~(1<<2)	/* Set MMC CS "low" */
